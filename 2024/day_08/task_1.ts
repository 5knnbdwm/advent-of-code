  ...
  for (let signal in signals) {
    for (let i = 0; i < signals[signal].length; i++) {
      ...
      for (let j = 0; j < signals[signal].length; j++) {
        ...
        const anti = { x: currentPosition.x + xDiff, y: currentPosition.y + yDiff }
        if (antinodes.includes(anti.x + ',' + anti.y)) continue
        if (anti.x < 0 || anti.x >= width || anti.y < 0 || anti.y >= height) continue
