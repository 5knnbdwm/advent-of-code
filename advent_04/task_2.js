...
eyr:2023
iyr:2017 byr:1994`
const inputTest = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
for (let i = 0; i < inputArr.length; i++) {
  const elements = inputArr[i].split(/\n|\s/);
  let validRows = 0

  for (let j = 0; j < elements.length; j++) {
    const row = elements[j].split(":")

    if (row[0] === "byr") {
      if (row[1].match(/^(19[2-9]\d|200[0-2])$/)) {  // solution from reddit
        // if (row[1].match(/^[12][90][2-90][0-9]$/)) {
        validRows++
      }
    }
    if (row[0] === "iyr") {
      // if (row[1].match(/^(20[1]\d|2020)$/)) { // solution from reddit
      if (row[1].match(/^20(1[0-9]|20)$/)) {
        validRows++
      }
    }
    if (row[0] === "eyr") {
      // if (row[1].match(/^(20[2]\d|2030)$/)) { // solution from reddit
      if (row[1].match(/^20(2[0-9]|30)$/)) {
        validRows++
      }
    }
    if (row[0] === "hgt") {
      // if (row[1].match(/(^1([5-8]\d|9[0-3])cm$)|(^([5-6]\d|7[0-6])in$)/)) { // solution from reddit
      if (row[1].match(/^(1(([5-8][0-9])|(9[0-3]))cm)|(((59)|(6[0-9])|(7[0-6]))in)$/)) {
        validRows++
      }
    }
    if (row[0] === "hcl") {
      // if (row[1].match(/^#[a-fA-F0-9]{6}$/)) { // solution from reddit
      if (row[1].match(/^#[0-9a-f]{6}$/)) {
        validRows++
      }
    }
    if (row[0] === "ecl") {
      // if (row[1].match(/^(amb|blu|brn|gry|grn|hzl|oth)$/)) { // solution from reddit
      if (row[1].match(/^amb|blu|brn|gry|grn|hzl|oth$/gm)) {
        validRows++
      }
    }
    if (row[0] === "pid") {
      // if (row[1].match(/^[0]*\d{9}$/)) { // solution from reddit
      if (row[1].match(/^\d{9}$/)) {
        validRows++
      }
    }
