...
/*
  ruleSet structure
  {
    the_bag_color: list of bags that bag color is included in
  }
*/
const ruleSet = {}

for (let i = 0; i < rules.length; i++) {
  const rule = rules[i].replace(/\sbags?/g, "").replace(/\s\d/g, "").replace(/\s/g, "_")
...
function checkBag(bagColor) {
  ...
  for (let i = 0; i < ruleSet[bagColor].length; i++) {
    m.push(ruleSet[bagColor][i])

    if (ruleSet[bagColor][i] in ruleSet) {
      const o = checkBag(ruleSet[bagColor][i])
      m = [...new Set([...m, ...o])]
    }
  }
...
console.log(checkBag(myBag).length)