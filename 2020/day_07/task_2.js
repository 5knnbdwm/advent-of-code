...
const inputTest = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
const inputTest2 = `shiny gold bags contain 2 dark red bags.
...
const myBag = "shiny_gold"
/*
  ruleSet structure
  {
    the_bag_color: list of bags that are inside of the_bag_color
  }
*/
const ruleSet = {}

for (let i = 0; i < rules.length; i++) {
  const rule = rules[i].replace(/\sbags?/g, "").replace(/\s/g, "_")
  const key = rule.split("_contain_")[0]
  const children = rule.split("_contain_")[1].replace(".", "").split(",_").filter((value) => value !== 'no_other')

  ruleSet[key] = children
}

function getBags(bagColor) {
  let m = 0

  for (let i = 0; i < ruleSet[bagColor].length; i++) {
    const count = Number(ruleSet[bagColor][i].replace(/(\d+).+/, `$1`))
    const nextBag = ruleSet[bagColor][i].replace(/\d_/, "")
    // number of ags inside of bag times number of times of the bag, plus the number of bags it self
    m += count * getBags(nextBag) + count
  }

  return m
}

console.log(getBags(myBag))